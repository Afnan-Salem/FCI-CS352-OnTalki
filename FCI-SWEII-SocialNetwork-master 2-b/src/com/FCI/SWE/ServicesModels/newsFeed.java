...
public class newsFeed extends TimeLine
{
	...
	public String getPosts() 
	{
		...
		DatastoreService datastore1 = DatastoreServiceFactory
				.getDatastoreService();
		Query gaeQuery1 = new Query("Pages Likers");
		...
		for (Entity entity : pq.asIterable()) 
		{
		    ...
		    if (entity.getProperty("privacy").toString().equals("PrivateController")&&
					!entity.getProperty("timeline").equals(email))
			{
				System.out.println("private");	
				String name = t.getFriend();
				String liker = t.getLikers();
				System.out.println("name = "+name);	
				if(entity.getProperty("timeline").equals(name)||entity1.getProperty("liker").equals(liker))
				{
					System.out.println("///////////////////// private////////");
					info.put("owner", entity.getProperty("owner"));
					info.put("post",entity.getProperty("content"));
			...
			if (entity.getProperty("privacy").toString().equals("CustomController")&&
					!entity.getProperty("timeline").equals(email))
			{
				System.out.println("private");	
	...
	public String getLikers()
	{
		DatastoreService datastore = DatastoreServiceFactory
				.getDatastoreService();
		String email = User.getCurrentActiveUser().getEmail();
		Query gaeQuery = new Query("Pages Likers");
		PreparedQuery pq = datastore.prepare(gaeQuery);
		JSONArray results = new JSONArray();
		for (Entity entity : pq.asIterable()) 
		{
			JSONObject info = new JSONObject();
			if(entity.getProperty("liker").equals(email))
				return entity.getProperty("liker").toString();
		}
		return null;
	}
